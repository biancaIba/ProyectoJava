        ...
        try {
            ...
            for (int i = 0; i < listaPublicaciones.getLength(); i++) {
                ...
                if (nodoPublicacion.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    String nombrePublicacion = elementoPublicacion.getElementsByTagName("nombrePublicacion").item(0).getTextContent();
                    String fechaSubida = elementoPublicacion.getElementsByTagName("fechaSubida").item(0).getTextContent();
                    LocalDate fechaSubidaLocalDate= LocalDate.parse(fechaSubida);
                    int cantMG = Integer.parseInt(elementoPublicacion.getElementsByTagName("cantMG").item(0).getTextContent());

                    if (elementoAudio != null) {
                        // Leer y procesar datos del audio
                        int velocidadBits = Integer.parseInt(elementoAudio.getElementsByTagName("velocidadBits").item(0).getTextContent());
                        ...
                        int ancho = Integer.parseInt(elementoImagen.getElementsByTagName("ancho").item(0).getTextContent());
                        int alto = Integer.parseInt(elementoImagen.getElementsByTagName("alto").item(0).getTextContent());
                        Imagen imagen = new Imagen(nombrePublicacion, fechaSubidaLocalDate, cantMG, resolucion, ancho, alto);
                        ...
                        int cantCuadros = Integer.parseInt(elementoVideo.getElementsByTagName("cantCuadros").item(0).getTextContent());
                        Video video = new Video(nombrePublicacion, fechaSubidaLocalDate, cantMG, resolucion, cantCuadros);
